...
import { SiFigma, SiReact, SiMongodb, SiTailwindcss } from 'react-icons/si';

const Education = () => {
  const controls = useAnimation();
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.1 }); // Changed to 0.1 for better mobile detection

  const skills = [
    { name: "UI/UX", level: 90, icon: <SiFigma /> },
    { name: "React", level: 85, icon: <SiReact /> },
    { name: "Tailwind", level: 85, icon: <SiTailwindcss /> },
    { name: "Backend", level: 70, icon: <SiMongodb /> },
  ];
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
          ...
          >
            <div>
              ...
              <p className="text-gray-300 text-xs sm:text-sm md:text-base leading-relaxed max-w-sm md:max-w-md mx-auto lg:mx-0">
                4th Year Information Systems Student focusing on bridging complex data and user-centric design.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 md:gap-4 max-w-xs md:max-w-sm mx-auto lg:mx-0">
              <div className="bg-white/5 p-3 md:p-4 rounded-2xl border border-white/10">
                <p className="text-blue-400 font-black text-lg md:text-0.5xl">2026</p>
          ...
          >
            <div>
              ...
              {skills.map((skill, i) => (
                <div key={i} className="mb-3 md:mb-4">
                  ...
                  <div className="h-1 bg-white/10 rounded-full mt-1 md:mt-2 overflow-hidden">
                    <motion.div
                      custom={skill.level}
            ...
            <div className="border-t border-white/10 pt-4 md:pt-6">
              <div className="flex gap-2 md:gap-4 p-3 md:p-4 bg-white/5 rounded-2xl border border-white/10">
                <FiTarget className="text-blue-400 text-lg md:text-xl mt-1" />
                <p className="text-[10px] sm:text-xs md:text-sm text-gray-300 leading-relaxed">
                  Aiming to specialize in Product Design and Full-Stack Engineering within Ethiopia's tech ecosystem.
                </p>
              </div>
            </div>
          </motion.div>
        ...
        <div className="mt-10 md:mt-16 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
          {[
            ...
            { val: "UI/UX", label: "Focus", icon: <SiFigma /> },
            ...
            <div key={i} className="bg-white/5 border border-white/10 rounded-2xl py-4 sm:py-6 text-center">
              <div className="text-blue-400 text-lg sm:text-xl mb-1">{s.icon}</div>
